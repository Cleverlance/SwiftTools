...
final class GetSimulatorIdUseCaseTests: XCTestCase {
    ...
    private var sut: GetSimulatorIdUseCaseImp!
    ...
    func test_givenBuildDestinationsAndiOSPlatform_whenTest_thenParseCorrectDestination() throws {
        let deviceId = try sut(for: .iOS, scheme: "scheme")

        XCTAssertEqual(deviceId, "40164398-DEA8-4D73-8813-CF7B2AC49090")
    }

    func test_givenBuildDestinationsAndmacOSPlatform_whenTest_thenParseCorrectDestination() throws {
        let deviceId = try sut(for: .macOS, scheme: "scheme")

        XCTAssertEqual(deviceId, "B717F26A-6C7A-5DE6-A1D0-9D0374071FD0")
    }
}
